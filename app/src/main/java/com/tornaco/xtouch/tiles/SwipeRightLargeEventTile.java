...
public class SwipeRightLargeEventTile extends QuickTile {

    private List<String> descList;

    private boolean firstHook = true;

    public SwipeRightLargeEventTile(Context context) {
        super(context);

        descList = EventRes.getDescriptions(context.getResources());

        this.titleRes = R.string.title_swipe_right;
        this.iconRes = R.drawable.ic_chevron_right_black_24dp;

        final int event = SettingsProvider.get().getInt(SettingsProvider.Key.SWIPE_RIGHT_LARGE_ACTION);
        ...
        this.tileView = new DropDownTileView(context) {
            ...
            protected void onItemSelected(int position) {
                ...
                SettingsProvider.get().putInt(SettingsProvider.Key.SWIPE_RIGHT_LARGE_ACTION, event);
