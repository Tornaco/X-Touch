...
public class ScreenToLEventTile extends QuickTile {

    private List<String> descList;

    private boolean firstHook = true;

    public ScreenToLEventTile(Context context) {
        super(context);

        descList = EventRes.getDescriptions(context.getResources());

        this.titleRes = R.string.title_screen_to_l;
        this.iconRes = R.drawable.ic_stay_primary_landscape_black_24dp;

        final int event = SettingsProvider.get().getInt(SettingsProvider.Key.SCREEN_TO_L_ACTION);
        ...
        this.tileView = new DropDownTileView(context) {
            ...
            protected void onItemSelected(int position) {
                ...
                SettingsProvider.get().putInt(SettingsProvider.Key.SCREEN_TO_L_ACTION, event);
